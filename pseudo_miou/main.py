...
################################
            ...
            # Validation  #
            ###############
            if self.epoch>=0 and batch_idx>=0 and batch_idx % 200 == 0:
                ...
                PA2, MPA2, MIoU2, FWIoU2 = self.validate_pseudo_best()

                print(PA, MPA, MIoU, FWIoU, "!!")
                print(PA2, MPA2, MIoU2, FWIoU2, "@@")
    ...
    @ torch.no_grad()
    def validate_pseudo_best(self, mode='target'):
        ...
        for val_idx, (x, y, id) in enumerate(tqdm(val_loader, desc=f"Val Epoch {self.epoch + 1}")):
            ...
            pseudo_best = label_1.cpu().numpy()

            self.evaluator.add_batch(label, pseudo_best)
