...
class Eval():
    ...
    # generate confusion matrix
    def __generate_matrix(self, gt_image, pre_image):
        gt_image = np.where(gt_image==-1, self.num_class, gt_image)
        pre_image = np.where(pre_image==-1, self.num_class, pre_image)

        mask = (gt_image >= 0) & (gt_image <= self.num_class)
        label = (self.num_class+1) * gt_image[mask].astype('int') + pre_image[mask]
        count = np.bincount(label, minlength=(self.num_class+1)**2)

        confusion_matrix = count.reshape((self.num_class+1), (self.num_class+1))
        confusion_matrix = confusion_matrix[:18,:18]
