...
class ResNetMulti(nn.Module):
    ...
    def forward(self, x, forward_target=2):
        input_size = x.size()[2:]
        ...
        x1 = self.layer5(x)
        pred_ori1 = nn.Upsample(size=(input_size[0], input_size[1]), mode='bilinear', align_corners=True)(x1)  # ResNet + ASPP1

        if forward_target > 0 :
            x3_1, x3_2 = getattr(self, TM_name1)(x)
            new_x = x1 + x3_1 + x3_2
            pred1 = nn.Upsample(size=(input_size[0], input_size[1]), mode='bilinear', align_corners=True)(new_x)  # ResNet + (ASPP1+TM1)

        x = self.layer4(x)
        x2 = self.layer6(x)
        pred_ori2 = nn.Upsample(size=(input_size[0], input_size[1]), mode='bilinear', align_corners=True)(x2)  # ResNet + ASPP2

        if forward_target > 0 :
            x3_1, x3_2 = getattr(self, TM_name2)(x)
            new_x = x2 + x3_1 + x3_2
            pred2 = nn.Upsample(size=(input_size[0], input_size[1]), mode='bilinear', align_corners=True)(new_x)  # ResNet + (ASPP2+TM2)
    ...
    def optim_parameters(self, cfg):
        # if cfg.from_scratch:
        #     optim_parameters = [{'params': self.ResNet_params(), 'lr': cfg.learning_rate},
        #                         {'params': self.ASPP_params(), 'lr': 10 * cfg.learning_rate}]
        #     if cfg.tm:
        #         optim_parameters += [{'params': self.TM_params(), 'lr': 10 * cfg.learning_rate}]
        # else:
        optim_parameters = [{'params': self.get_1x_lr_params_NOscale(), 'lr': cfg.opt.learning_rate},
                            {'params': self.get_10x_lr_params(), 'lr': cfg.opt.learning_rate}]
        if self.tm:
            optim_parameters += [{'params': self.TM_params(), 'lr': 10 * cfg.opt.learning_rate}]
