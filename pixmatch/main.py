...
class Trainer():
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (batch_s, batch_t) in enumerate(tqdm(
            ...
            #     pred_1, pred_2 = unpack(pred)
            #     pred_prev = self.model(x.to(self.device), forward_target=0)
            #     pred_prev0_1, pred_prev0_2 = unpack(pred_prev)
            #     pred_prev = self.model(x.to(self.device), forward_target=1)
            #     pred_prev1_1, pred_prev1_2 = unpack(pred_prev)


            #     pred_prev0_1 = F.softmax(pred_prev0_1, dim=1)
            #     pred_prev1_1 = F.softmax(pred_prev1_1, dim=1)
            #     pred_P_1 = F.softmax(pred_1, dim=1)
            #     pred_P_1 = (pred_P_1 + pred_prev0_1 + pred_prev1_1) / 3.0
            ...
            #     if self.cfg.aux:
            #         pred_prev0_2 = F.softmax(pred_prev0_2, dim=1)
            #         pred_prev1_2 = F.softmax(pred_prev1_2, dim=1)
            #         pred_P_2 = F.softmax(pred_2, dim=1)
            #         pred_P_2 = (pred_P_2 + pred_prev0_2 + pred_prev1_2) / 3.0
            ...
            if self.cfg.lam_aug > 0:
                ...
                del pred_aug, pred_aug_1, pred_aug_2, loss_aug, loss_aug_1, loss_aug_2, loss_new_aug
    ...
    def validate(self, mode='target'):
        ...
        if self.cfg.data.source.kwargs.class_16:
            ...
            self.writer.add_scalar('MIoU', MIoU, self.epoch)
            self.writer.add_scalar('FWIoU', FWIoU, self.epoch)
        # if self.cfg.data.target.dataset == 'IDD':
        #     MIoU=(MIoU*19)/18
