...
class Trainer():
    def __init__(self, cfg, logger, writer):
        ...
        saved_state_dict0 = torch.load("/data/shinahyung/code/3CUDA/0509/t1/GTA5_150000.pth", map_location=self.device)
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (batch_s, batch_t) in enumerate(tqdm(
            ...
            with torch.no_grad():
                ...
                # pseudo_outputs0_1, pseudo_outputs0_2 = self.old_lst[id[0]], self.old_lst[id[0]]
                # pseudo_outputs0_1 = torch.from_numpy(pseudo_outputs0_1).to(self.device)
                # pseudo_outputs0_2 = torch.from_numpy(pseudo_outputs0_2).to(self.device)
                pseudo_outputs1_1, pseudo_outputs1_2 = self.model(x.to(self.device), forward_target=self.cfg.num_target) # Source + TM3
